...
static long getLong(long* pResultado)
{
    long retorno=-1;
    char bufferString[50];
    if(	pResultado != NULL &&
...
int utn_getCuit(long* pResultado, char* mensaje, char* mensajeError, long minimo, long maximo, int reintentos)
{
	int retorno = -1;
	long bufferInt;
	do
...
int utn_getEleccion(int* pResultado,char* mensaje, char*mensajeError, int reintentos)
{
	int retorno = -1;
		int bufferInt;
		do
...
int utn_verificarNumerica(char* cadena, int limite)
{
	int retorno = -1;
	if(cadena != NULL && limite > 0)
	{
